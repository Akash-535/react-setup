...
const Hero = () => {
    return (
        <div className='bg-hero-bg max-2xl:bg-contain bg-cover max-xl:bg-cover bg-top bg-no-repeat max-lg:pb-4 max-2xl:px-5'>
            <Header />
            <div className='container mt-[120px] max-lg:mt-16 max-md:mt-9 max-sm:mt-4'>
                    ...
                    <div className="w-6/12 max-lg:w-full max-lg:text-center flex justify-between flex-col">
                        <div>
                            ...
                            <div className='flex p-[9px] pl-[22px] bg-white rounded-[0px_50px_50px_20px] items-center max-w-[476px] justify-between max-sm:pl-3 max-lg:mx-auto max-sm:gap-1'>
                                <input type="text" placeholder='Start typing your address' className='placeholder:text-deep-blue placeholder:text-base placeholder:font-normal !outline-none !bg-transparent text-deep-blue text-base font-normal placeholder:max-sm:text-sm max-sm:text-sm w-full' />
                                <Button customButton="Compare" buttonSpan={<ButtonArrowIcon />} />
                            </div>
                        </div>
                        <div>
                            <p className='text-custom-3xl font-normal leading-custom-2xl max-md:text-3xl max-sm:text-2xl max-lg:pt-5 max-xl:text-white 2xl:text-white'>More than <span className='text-pacific-blue font-bold'>80,000</span>+ companies trust bill central </p>
                        </div>
                    </div>
                    <div className='w-6/12 max-lg:w-full max-lg:items-center max-lg:flex max-lg:justify-center'>
                        <div className='relative'>
                            <img className='absolute -top-14 -right-[9%] max-sm:hidden max-lg:-right-10 max-lg:-top-3' src={dotsImg} alt="dots" />
                            <img className='max-w-[558px] max-lg:max-w-[490px] w-full relative z-[1] rounded' src={heroImg} alt="img" />
                        </div>
                    </div>
                </div>
            </div>
