...
const CompaniesTrust = () => {
    return (
        <div>
            <div className='container px-5 max-xl:pt-14 max-md:pt-10 2xl:pt-14'>
                ...
                <div
                    className='pt-16 flex gap-[51px] items-center justify-between max-md:pt-8 max-sm:pt-5 max-xl:overflow-x-auto overflow-hidden'>
                    {COMPANIES_LOGO_LIST.map((obj, i) => (
                        <div key={i} className='hover:scale-105 duration-300 ease-linear'>
                            <img className={`${i === 0 ? 'max-w-[127px]' : i === 1 ? 'max-w-[102px]' : i === 2 ? 'max-w-[94px] max-h-[32px]' : i === 3 ? 'max-w-[49px] ' : i === 4 ? 'max-w-[62px]' : i === 5 ? 'max-w-[84px]' : i === 6 ? 'max-w-[63px]' : ''}`} src={obj} alt="companies logo" />
                        </div>
                    ))}
                </div>
            </div>
